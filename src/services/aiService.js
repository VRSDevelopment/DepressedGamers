...
export const generateAIResponse = async (guildId, userMessage, systemPrompt) => {
  if (!aiClient) {
    if (!initAIClient()) {
  ...
  try {
    ...
    const promptInstructions = systemPrompt || "You are a helpful, friendly, and concise Discord bot. Respond directly to the user's messages without adding formatting unless it's useful.";

    const contents = [];
    
    // Convert history into contents array for Gemini
    // @google/genai format uses { role: 'user' | 'model', parts: [{text: '...'}] }
